    ...
    def run(self):
        ...
        while i < 10 :
            with verrou :
                ...
                if self.arduino['em_timer'] >= 60 :
                    message1 = "mechanism status : 60s elapsed since last send"
                    minuteur_em_reset = 1
                    
                if reponse[3] == "T" and self.arduino['em_status'] != "OK" :
                    self.arduino['em_status'] = "OK"
                    message1 = "MECHANISM STATUS HAS CHANGED"
                    minuteur_em_reset = 1
                
                if minuteur_em_reset == 1 :
                    print("--- Mechanism %s ---\n%s\n--- Envoie au serveur ---"  %(reponse[0], message1))
                    self.arduino['em_timer'] = 0
                    thread_minuteur_em = MinuteurEM(self.arduino)
