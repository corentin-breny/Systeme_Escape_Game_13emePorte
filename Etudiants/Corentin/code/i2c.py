...
def send_SDtoDataBase(arduino, console_message):
    ...
    for sensor_name in arduino['sensor_data'] :                     #Pour chaque capteur du mecanisme
        cursor.execute("Insert INTO capteurs (ID_Capteurs, Nom_Capteurs, Valeur) VALUES (%s, %s, %s)", (arduino['id'], sensor_name, arduino['sensor_data'][sensor_name])
        ...
        #sensor_name : correspond au nom du capteur
        #arduino['sensor_data'][sensor_name] : correspond a la valeur du capteur
...
def send_AStoDataBase(arduino, console_message):
    ...
    for actuator_name in arduino['actuator_status'] :               #Pour chaque actionneur du mecanisme
        cursor.execute("Insert INTO actionneurs (ID_Actionneur, Nom_Actionneur, Etat) VALUES (%s, %d, %s)", (arduino['id'], actuator_name, arduino['actuator_status'][actuator_name])
        conn.commit()	#sert à sauvegarder les données dans la table avant de fermer la connexion. 
        #Envoyer en BDD sachant que :
        #arduino['id'] : correspond a l'id de l'Arduino
...
def send_MStoDataBase(arduino, console_message):
    ...
    """
    cursor.execute("Insert INTO general (ID_mecanisme, Etat) VALUES (%s, %s)", (arduino['id'], arduino['mechanism_status'])
    conn.commit()	#sert à sauvegarder les données dans la table avant de fermer la connexion. 
    #Envoyer en BDD sachant que :
    #arduino['id'] : correspond a l'id de l'Arduino
